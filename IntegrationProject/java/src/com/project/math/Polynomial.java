 ...
 * @param <C>
 */
public class Polynomial<E extends Integer, C extends Double> extends Function<E, C> implements Cloneable{
		...
		for(int index = 0; index < exponents.size(); index++) {
			if(polyString.length() == 0) {
				if(coefficients.get(index) != 0) {
					if(coefficients.get(index) > 1 || (coefficients.get(index) < 1 && coefficients.get(index) > 0)) {
						polyString.append(coefficientToString(coefficients.get(index)) + exponentToString(exponents.get(index)));
					}
